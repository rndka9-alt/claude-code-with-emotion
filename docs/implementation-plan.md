...
- Claude session startup now also appends a visual-tool usage prompt assembled from dedicated prompt files, so the rule for when to call `set_visual_overlay` (with `emotion` and/or `line`) is editable without hardcoding another giant string into the wrapper.
