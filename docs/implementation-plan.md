...
- Claude session startup now also appends a visual-tool usage prompt assembled from dedicated prompt files, so the rule for when to call `set_visual_emotion`, `set_visual_line`, and `clear_visual_line` is editable without hardcoding another giant string into the wrapper.
