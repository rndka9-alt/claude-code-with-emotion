...
function replacePlistValue(contents, key, value) {
  ...
  return contents.replace(pattern, `<key>${key}</key>\n\t<string>${value}</string>`);
}
...
copyRuntimeTree(path.join(projectRoot, 'dist', 'main'), path.join(resourcesAppPath, 'dist', 'main'));
copyRuntimeTree(path.join(projectRoot, 'dist', 'preload'), path.join(resourcesAppPath, 'dist', 'preload'));
copyRuntimeTree(path.join(projectRoot, 'dist', 'renderer'), path.join(resourcesAppPath, 'dist', 'renderer'));
copyRuntimeTree(path.join(projectRoot, 'node_modules'), path.join(resourcesAppPath, 'node_modules'));
copyRuntimeTree(path.join(projectRoot, 'bin'), path.join(resourcesAppPath, 'bin'));
